...
Model_C_sim <- function(N){
  ...
  return(arima.sim(n = N, list(ar = c(0.9)),
                   sd = sqrt(1-0.9^2)))
...
par(mar = c(3, 2, 2, 1),
    lwd = 1.5)
par(oma = c(1,0,0,0))
plot(YA, type = "l", main = "(a)", xaxt = 'n', 
     lwd = 1.5)
axis(1, labels = FALSE)
plot(YB, type = "l", main = "(b)", xaxt = 'n',
     lwd = 1.5)
axis(1, labels = FALSE)
plot(YC, type = "l", main = "(c)", xlab = "Índice", mgp=c(2,1,0),
     lwd = 1.5)
plot(YD, type = "l", main = "(d)", xlab = "Índice", mgp=c(2,1,0),
     lwd = 1.5)
