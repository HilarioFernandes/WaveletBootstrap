...
plot(X_oceanshear, type = "l", xaxt = "n", xlab = "Profundidade (metros)",
     ylab = "Cisalhamento \n (diferenciado)",
     lwd = 1.5)
abline(v = seq(0.5,6656.5, by = block_length), lty = "dashed",
       lwd = 1.5)
axis(1, at = seq(1,length(X_oceanshear), length.out = 14), 
...
returns_fun <- function(data){
  ...
  names_temp <- apply(names_temp,1,function(x){paste0(x[2], "_", x[1])})
...
plot(X_CBOE, type = "l", xaxt = "n", xlab = "Horário (hh:mm)", ylab = "Retorno",
     ylim= c(-0.001,0.001),
     lwd = 1.5)
abline(v = seq(0.5,2048.5, by = block_length), lty = "dashed",
       lwd = 1.5)
axis(1, at = seq(1,length(X_CBOE), length.out = 17), 
...
latex_fun <- function(results){
  ...
  temp <- arrange(distinct(as.data.frame(rbind(temp[[1]], temp[[2]],
                                                temp[[3]], temp[[4]]))),
                   V2,V1,V3)
