...

{
  ...
  unlist(lapply(best_clusters, function(x){adj.rand.index(x, true_cluster)}))
...
main_fun <- function(iterations, B, n, n1, t, model){
  ...
  for(i in 1:iterations){
    ...
    output_wv_qU <- rbind(output_wv_qU,
                    unlist(lapply(best_clusters, function(x){adj.rand.index(x, true_cluster)})))
...
quantiles_fun <- function(list_obj, rounding){
  ...
  return(round(cbind(matrix(unlist(lapply(list_obj[2:5], quantile)), nrow = 5),
                     apply(list_obj[[1]], 2, quantile)),rounding))
...
Latex_out_fun <- function(list_obj12, list_obj14, list_obj16, rounding, t, q, mod){
  ...
  cat(paste0("\\caption{Quartis empíricos dos índices ajustados de Rand para $t = ",  t  ,"$, ",
