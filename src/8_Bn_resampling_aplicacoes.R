...
Bn_resampling_tests <- function(X,Y,alpha,B){
  ...
  normal_approx_CI_jackknife <- qnorm(1-0.05, mean = 0,
                                      sd = sd(statistics_jackknife)*(2*n-1)/(sqrt(2*n))) 
  
  output1 <- c(statistic, quantile_CI_boot_discy, quantile_CI_boot_discn,
              normal_approx_CI_boot_discy, normal_approx_CI_boot_discn,
              normal_approx_CI_jackknife)
  
  names(output1) <- c("statistic","quant_boot_disc","quant_boot_no_disc",
                     "normal_boot_disc","normal_boot_no_disc",
                     "normal_jackknife")
  
  output2 <- c(sum(statistics_boot_discy > statistic)/B,
