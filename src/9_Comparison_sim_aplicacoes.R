...
comparison_fun <- function(X, Y, alpha, B) {
  ...
  for (jmax in 1:n_levels) {
    ...
    quants_boot_dep <- t(apply(ratios_boot_dep, 2, function(x) {
    ...
    quants_boot_indep <- t(apply(ratios_boot_indep, 2, function(x) {
      quantile(x, c(alpha / 2, 1 - alpha / 2))
    })[, 1:jmax])
...
sample_arrowhead <- cbind(
  arrowhead_list_diff[[1]], arrowhead_list_diff[[2]],
