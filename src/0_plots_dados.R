...
plot(ocean_shear, type = "l", xaxt = "n", xlab = "Profundidade (metros)",
     ...
     lwd = 1.5)
axis(1, at = seq(1, 6875, length.out = 10),
...
plot(oceanshear_diff, type = "l", xaxt = "n", xlab = "Profundidade (metros)",
     ylab = "Cisalhamento \n (diferenciado)", yaxt = "n",
     lwd = 1.5)
axis(1, at = seq(2, 6875, length.out = 10),
...
plot(oceanshear_diff.modwt[[1]], type = "l", xaxt = "n", yaxt = "n", ylab = "",
     lwd = 1.5)
axis(1, labels = FALSE, at = seq(2, 6875, length.out = 10))
title(ylab=expression(W[list(1,t)]), mgp=c(2,1,0))

plot(oceanshear_diff.modwt[[2]], type = "l", xaxt = "n", yaxt = "n", ylab = "",
     lwd = 1.5)
axis(1, labels = FALSE, at = seq(2, 6875, length.out = 10))
title(ylab=expression(W[list(2,t)]), mgp=c(2,1,0))

plot(oceanshear_diff.modwt[[3]], type = "l", xaxt = "n", yaxt = "n", ylab = "",
     lwd = 1.5)
axis(1, labels = FALSE, at = seq(2, 6875, length.out = 10))
title(ylab=expression(W[list(3,t)]), mgp=c(2,1,0))

plot(oceanshear_diff.modwt[[4]], type = "l", xaxt = "n", yaxt = "n", ylab = "",
     lwd = 1.5)
axis(1, labels = FALSE, at = seq(2, 6875, length.out = 10))
title(ylab=expression(W[list(4,t)]), mgp=c(2,1,0))

plot(oceanshear_diff.modwt[[5]], type = "l", xaxt = "n", yaxt = "n", ylab = "",
     lwd = 1.5)
axis(1, labels = FALSE, at = seq(2, 6875, length.out = 10))
title(ylab=expression(W[list(5,t)]), mgp=c(2,1,0))

plot(oceanshear_diff.modwt[[6]], type = "l", xaxt = "n", yaxt = "n", ylab = "",
     lwd = 1.5)
axis(1, at = seq(2, 6875, length.out = 10),
     labels = round(seq(350.1, 1037.4, length.out = 10)))
title(ylab=expression(W[list(6,t)]), mgp=c(2,1,0),
...
plot(CBOE_selection_temp, type = "l", xlab = "Data (dia.mês)",
     ylab = "", xaxt = "n",
     lwd = 1.5)
axis(2, labels = FALSE)
axis(1, at = seq(1,43*28, by = 7*28),
     labels = labels_plot)
...

{
  ...
  plot(CBOE_selection_returns, type = "l", xlab = "Data (dia.mês)",
       xaxt = "n", ylab= "Retorno",
       lwd = 1.5)
  axis(1, at = seq(1,43*28, by = 7*28),
       labels = labels_plot)
...
par(mar = c(4.1, 5, 1, 1))
par(mfrow=c(1,1))
plot(MJO_paper, type = "l", xaxt = "n", ylim = c(-4,4), xlab = "Ano",
...
par(mar = c(4, 5, 1, 1))
par(mfrow = c(2, 1))
plot(arrowhead[1:753], type = "l", xlab = "Índice (Avonlea)", ylab = "Distância",
...
plot(arrowhead[754:1506], type = "l", xlab = "Índice (Clovis)", ylab = "Distância",
     lwd = 1.5, ylim = c(-2.5,3), yaxt = "n", xaxt="n")
abline(v=indexes_min, col = "black", lty = 2)
...
par(mfrow=c(1,1))
plot(INMET_selec[,3], type = "l", ylim = c(9,35), 
...
axis(1,at = c(first_days, length(INMET_selec[,3]+1)), labels = c("Jan", "Fev", "Mar",
                                                    "Abr", "Mai", "Jun",
                                                    "Jul", "Ago", "Set",
                                                    "Out", "Nov", "Dez", "Jan"))
legend("bottomleft", legend=c("Altamira", "Maringá"),  
       col = c("black","grey"),
       lty = c("solid", "solid"),
       lwd = 1.5)
...
boot_example_bootstrap = c(boot_example_extended[block_start_points[1]:(block_start_points[1]+block_lengths[1]-1)],
